...
def f_stat(model):
    """
    Take an object under Linear_Model and return its f-statistic.
    ...
    """
    MSE = model.utu
    RSE = np.sum((model.predy - model.mean_y) ** 2)
    fStat = (RSE / (model.k - 1)) / (MSE / (model.n - model.k))
    pValue = stats.f.sf(fStat, model.k - 1, model.n - model.k)
    fs_result = (fStat, pValue)
    return fs_result


def t_stat(model, z_stat=False):
    """
    it's a t-stat, yo
    """

    variance = model.vm.diagonal()
    tStats = model.betas[range(0, len(vm))].reshape(len(model.vm),) / np.sqrt(variance)
    
    if z_stat:
        return [(t, stats.norm.sf(abs(t))*2) for t in tStats]
    else:
        return [(t, stats.t.sf(abs(t), model.n-model.k)*2) for t in tStats]

def r2(model):
    """
    R2 is a measure of model fit.
    """                     #eventually, the things that used to be here should
                            #get pushed into type annotations?
    ss_tot = ((model.y - model.mean_y) ** 2).sum(0)
    r2 = 1 - model.utu / ss_tot 
    return float(r2)
...
def ar2(model):
    ...
    """
    return  1 - (1 - r2(model)) * (model.n - 1) / (model.n - model.k)
...
def log_likelihood(model):
    ...
    """
    ll_result = -0.5 * (n * (np.log(2 * math.pi)) + \
                model.n * np.log(model.utu / model.n) + \
                (model.utu / (model.utu / model.n)))
    return ll_result
...
def schwarz(model):
    ...
    try:  # ML case logll already computed
        ...
        sc_result = model.k * np.log(model.n) - 2.0 * model.logll
    except AttributeError:          # OLS case
        utu = reg.utu  # (scalar) residual sum of squares
        sc_result = model.k * np.log(model.n) + n * (np.log((2 * np.pi *
            model.utu) / model.n) + 1)
    return sc_result
...
def condition_index(reg):
    ...
    scale = xtx / np.diagonal(xtx)
    eigval = np.linalg.eigvals(scale)
    return  sqrt(max(eigval) / min(eigval))
...
def jarque_bera(reg):
    ...
    mu2, mu3, mu4 = (np.mean(reg.u**i) for i in range(2,5)) #auto unpack is neat
